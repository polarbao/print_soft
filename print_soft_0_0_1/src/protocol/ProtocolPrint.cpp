	...
	:QObject(parent)
{
	...
	//qRegisterMetaType<PrintSpeedParam>("PrintSpeedParam");
}

void ProtocolPrint::SetSerialPort(TcpClient* serialport)
{
	//更改数据
...
QByteArray ProtocolPrint::GetResponseDatagram(FunCode code, QByteArray data /*= QByteArray()*/)
{
	const int size = 100;
	uchar sendBuf[size];
	memset(sendBuf, 0, size);
...
void ProtocolPrint::Parse(QByteArray& datagram)
{
	...
	//比较包头
	if (!(recvBuf[0] == HI_OF_SHORT(Active_Head) && (recvBuf[1] == LO_OF_SHORT(Active_Head))))
	{
		LOG_DEBUG(u8"数据包头错误。");
		return;
	}
	...
	//如果是下位机回应上位机的包
	if (recvBuf[1] == LO_OF_SHORT(Active_Head))
	{
		//Note:解析回复数据
		//回复的命令最高位-0x1000
