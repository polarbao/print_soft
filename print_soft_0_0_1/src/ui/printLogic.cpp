...
PrintSimulateHandle::PrintSimulateHandle(QWidget *parent)
    : QObject(parent)
...
PrintSimulateHandle::~PrintSimulateHandle()
{


}

void PrintSimulateHandle::Init() 
{
	...
	//net
	connect(m_netBtnGroup, QOverload<int>::of(&QButtonGroup::buttonClicked), this, &PrintSimulateHandle::OnListenBtnClicked);

	//motion
	connect(m_printOperBtnGroup, QOverload<int>::of(&QButtonGroup::buttonClicked), this, &PrintSimulateHandle::OnHandlePrintOperFun);
	connect(m_printParamBtnGroup, QOverload<int>::of(&QButtonGroup::buttonClicked), this, &PrintSimulateHandle::OnHandlePrintParamFun);	
	connect(m_printMoveBtnGroup, QOverload<int>::of(&QButtonGroup::buttonClicked), this, &PrintSimulateHandle::OnHandlePrintMoveFun);
	
	//UI
	connect(this, &PrintSimulateHandle::SigAddShowOperCmd, this, &PrintSimulateHandle::OnShowOperCmd);
...
void PrintSimulateHandle::InitUI()
{
	...
	QGroupBox* box21 = new QGroupBox(QString("打印参数设置"));
	{
		QGridLayout* layout = new QGridLayout;
		...
		addUI(u8"清洗位置", "EPPT_SetClenaPos", EPPT_SetClenaPos, 0, 6, true);
	...
	QGroupBox* box31 = new QGroupBox(QString("打印移动操作"));
	{
		QGridLayout* layout = new QGridLayout;
	...
	QGroupBox* box4 = new QGroupBox(QString("操作信息"));
	{
		QGridLayout* layout = new QGridLayout;
	...
	QGroupBox* box5 = new QGroupBox(QString("发送/接收信息"));
	{
		QGridLayout* layout = new QGridLayout;
...
void PrintSimulateHandle::SetMCCfgFile2UI(const MotionConfig& data)
{
	...
	this->findChild<QLineEdit*>("EPPT_SetStopPos")->setText ((data.startPos.toStrList()).join(","));
	this->findChild<QLineEdit*>("EPPT_SetClenaPos")->setText((data.startPos.toStrList()).join(","));
...
void PrintSimulateHandle::OnListenBtnClicked(int idx)
{
	QString logStr;
...
void PrintSimulateHandle::OnShowOperCmd(const QString& msg, QByteArray arr, const ShowEditType& type)
{
	QString showStr = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss:zzz");
	showStr.append("\n");
	switch (type)
	{
	case PrintSimulateHandle::ESET_OperComm:
	{
	...
	case PrintSimulateHandle::ESET_Sendomm:
	{
		showStr.append(QString("当前操作："));
		showStr.append(msg + "\n");
		showStr.append(arr + "\n");
	...
	case PrintSimulateHandle::ESET_RecvComm:
	{
...
void PrintSimulateHandle::OnHandlePrintOperFun(int idx)
{
...
void PrintSimulateHandle::OnHandlePrintParamFun(int idx)
{
	auto btnType = static_cast<EPrintOperFun>(idx);
	...
	switch (btnType)
	{
		...
		case EPPT_SetStepX:
		{
			...
			hexArr = Utils::GetInstance()->MultiMicroDisStrTo12BytesHex(valData, false, false);
			operStr.append(QString("设置X轴步进值"));
			logStr.append(QString("设置打印终点信息"));
			logStr.append(QString("数据区发送数据：%1").arg(QString(hexArr.toHex(' ').toUpper())));
		...
		case EPPT_SetStepY:
		{
			...
			hexArr = Utils::GetInstance()->MultiMicroDisStrTo12BytesHex(valData, false, false);
			operStr.append(QString("设置Y轴步进值"));
			logStr.append(QString("设置打印终点信息"));
			logStr.append(QString("数据区发送数据：%1").arg(QString(hexArr.toHex(' ').toUpper())));
...
void PrintSimulateHandle::OnHandlePrintMoveFun(int idx)
{
