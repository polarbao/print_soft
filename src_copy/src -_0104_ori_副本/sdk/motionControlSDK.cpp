...
motionControlSDK::~motionControlSDK()
{
	...
	// 将配置信息重新写入到配置文件
	CMotionConfig::GetInstance().save();
...
bool motionControlSDK::MC_Init(const QString& logDir)
{
	...
	//SPDLOG_INFO(QString(u8"SDK初始化成功"));
...
bool motionControlSDK::MC_Connect2Dev(const QString& ip, quint16 port)
{
	...
	if (!d->initialized) 
	{
		...
		//SPDLOG_INFO(u8"SDK未初始化，请先调用MC_Init()");
		return false;
	}
...
bool motionControlSDK::loadMotionConfig(MotionConfig & config, const QString & path)
{
	if (!CMotionConfig::GetInstance().load(path))
	{
		return false;
	}

	config = CMotionConfig::GetInstance().config();
	return true;
}
